...
export const Route = createFileRoute("/shop")({
  head: () => ({
    meta: [
      ...
      {
        property: "og:title",
        content: "Healthcare Store — LifeGuard AI",
      },
      {
        property: "og:description",
        content:
          "Order AI-recommended medicines, supplements and monitoring devices.",
      },
      {
        name: "robots",
        content: "noindex",
...
function ShopPage() {
  ...
  async function checkout() {
    ...
    try {
      ...
      toast.success(
        `Order ${order.id} placed — ${order.items} items, ₹${order.total}, arriving in ${order.eta}`,
      );
        ...
        <GlassPanel className="h-fit p-6 sm:p-8">
          ...
          {lines.length === 0 ? (
            ...
            <p className="mt-4 text-sm text-muted-foreground">
              Cart is empty. Add a recommended product to continue.
            </p>
